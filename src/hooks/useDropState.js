...
export function useDropDerived({ drops, bossState, activeDropId, isNear, getCooldown }) {
  ...
  useEffect(() => {
    ...
    let dkd_completed_value = false;

    const dkd_sort_entries_value = () => visibleDropEntries
      .slice()
    ...
    const dkd_commit_sorted_value = () => {
      if (dkd_cancelled_value || dkd_completed_value) return;
      dkd_completed_value = true;
      setSortedVisibleEntries(dkd_sort_entries_value());
      setDeferredPreviewReady(true);
    };
    ...
    const dkd_fallback_id_value = setTimeout(dkd_commit_sorted_value, 48);
    const dkd_interaction_task_value = InteractionManager.runAfterInteractions(() => {
      clearTimeout(dkd_fallback_id_value);
      dkd_commit_sorted_value();
    });
