...
const COURIER_DOCS_BUCKET = 'lootonia-courier-docs';
...
export async function submitCourierApplication({ userId, form }) {
  ...
  const city = String(payload?.city || 'Ankara');
  ...
  const profilePatch = {
    ...
    courier_profile_meta: {
      application_version: '2026-04-05',
  ...
  if (profileWrite?.error) {
    ...
    if (!message.includes('courier_city') && !message.includes('courier_zone') && !message.includes('courier_vehicle_type') && !message.includes('courier_profile_meta')) {
