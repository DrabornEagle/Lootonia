...
export async function deleteAdminCourierJob(jobId) {
  ...
  let res = await supabase.rpc('dkd_admin_courier_job_delete', { dkd_job_id: dkd_job_id_value });
  if (!res?.error) return res;
  if (!String(res?.error?.message || '').toLowerCase().includes('schema cache')) return res;
  return supabase.rpc('dkd_admin_courier_job_delete', { dkd_param_job_id: dkd_job_id_value });
}
...
function dkd_non_null_template_text_value(value) {
