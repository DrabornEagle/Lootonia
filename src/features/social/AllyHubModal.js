...
export default function AllyHubModal({ visible, onClose, sessionUserId, profile, refreshProfile }) {
  ...
  const incoming = Array.isArray(snapshot?.incoming) ? snapshot.incoming : [];
  const outgoing = Array.isArray(snapshot?.outgoing) ? snapshot.outgoing : [];
                ...
                {tab === 'friends' ? (
                  <>
                    ...
                    <View style={dkd_styles.panel}>
                      ...
                      <View style={dkd_styles.stackGap}>
                        {incoming.length
                          ? incoming.map((row) => <RequestRow key={`in-${row?.request_id}`} row={row} mode="incoming" onAccept={() => handleReply('accept', row)} onReject={() => handleReply('reject', row)} />)
                    ...
                    <View style={dkd_styles.panel}>
                      ...
                      <View style={dkd_styles.stackGap}>
                        {outgoing.length
                          ? outgoing.map((row) => <RequestRow key={`out-${row?.request_id}`} row={row} mode="outgoing" />)
