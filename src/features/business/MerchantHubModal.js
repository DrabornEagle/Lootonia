...
export default function MerchantHubModal({ visible, onClose }) {
  ...
  const daily = Array.isArray(dashboard?.daily) ? dashboard.daily.filter((dkd_numeric_value) => Number(dkd_numeric_value?.scan_count || 0) > 0) : [];
  ...
  const recentUses = Array.isArray(dashboard?.recentUses) ? dashboard.recentUses.slice(0, 8) : [];
  const products = Array.isArray(businessProducts) ? businessProducts : [];
