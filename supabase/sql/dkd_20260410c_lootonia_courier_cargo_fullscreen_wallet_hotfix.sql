...
immutable
as $$
  select round(greatest(coalesce(dkd_param_courier_fee_tl, 0), 0), 2);
$$;
...
create or replace function public.dkd_cargo_delivery_seed_text(
  dkd_param_pickup_address_text text default null,
  dkd_param_delivery_address_text text default null,
  dkd_param_extra_text text default null
)
returns text
language sql
immutable
  ...
  select coalesce(
    nullif(
      concat_ws(
        ...
        nullif(trim(coalesce(dkd_param_extra_text, '')), '')
      ),
      ''
    ),
    'cargo-delivery'
...
create or replace function public.dkd_cargo_delivery_random_fee_from_seed(
  dkd_param_seed_text text default null
)
returns numeric
language plpgsql
immutable
as $$
declare
  dkd_seed_value text := coalesce(dkd_param_seed_text, 'cargo-delivery');
  dkd_hash_value bigint := 0;
  dkd_index_value integer := 0;
begin
  if char_length(dkd_seed_value) = 0 then
    dkd_seed_value := 'cargo-delivery';
  end if;

  for dkd_index_value in 1..char_length(dkd_seed_value) loop
    dkd_hash_value := mod(
      dkd_hash_value + (ascii(substr(dkd_seed_value, dkd_index_value, 1)) * dkd_index_value),
      2147483647
    );
  end loop;

  return 40 + mod(dkd_hash_value, 31);
end;
$$;
...
as $$
  select round(
    ...
    + coalesce(public.dkd_cargo_delivery_random_fee_from_seed(dkd_param_seed_text), 0),
    2
  );
$$;
...
immutable
as $$
  select public.dkd_cargo_total_fee_from_distance_km(
    dkd_param_pickup_distance_km,
    dkd_param_delivery_distance_km,
    null
  );
$$;
...
drop function if exists public.dkd_courier_job_accept(bigint, numeric, numeric);
create function public.dkd_courier_job_accept(
...
begin
  ...
  if dkd_param_job_id is null or (dkd_cargo_shipment_id is null and not exists (
    select 1 from public.dkd_courier_jobs where id = dkd_param_job_id and assigned_user_id = dkd_user_id
  )) then
  ...
  dkd_live_lat := dkd_param_live_lat;
  dkd_live_lng := dkd_param_live_lng;
...
as $$
begin
  return public.dkd_courier_job_accept(dkd_param_job_id, null::numeric, null::numeric);
end;
$$;
...
begin
  ...
  returning coalesce(public.dkd_courier_jobs.fee_tl, 0),
            ...
            public.dkd_courier_jobs.pickup,
            public.dkd_courier_jobs.dropoff
  into dkd_fee_tl, dkd_reward_score, dkd_eta_min, dkd_cargo_shipment_id, dkd_pickup_distance_km, dkd_pickup_lat, dkd_pickup_lng, dkd_dropoff_lat, dkd_dropoff_lng, dkd_pickup_text, dkd_dropoff_text;
