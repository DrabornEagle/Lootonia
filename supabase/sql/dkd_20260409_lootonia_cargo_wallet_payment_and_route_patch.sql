...
create or replace function public.dkd_cargo_customer_charge_from_courier_fee(
...
update public.dkd_cargo_shipments as dkd_alias_cargo_shipment
set courier_fee_tl = case
      when coalesce(dkd_alias_cargo_shipment.courier_fee_tl, 0) > 0 then dkd_alias_cargo_shipment.courier_fee_tl
      else coalesce(public.dkd_courier_jobs.fee_tl, 0)
    end,
    customer_charge_tl = case
      when coalesce(dkd_alias_cargo_shipment.customer_charge_tl, 0) > 0 then dkd_alias_cargo_shipment.customer_charge_tl
      else public.dkd_cargo_customer_charge_from_courier_fee(
        case
          when coalesce(dkd_alias_cargo_shipment.courier_fee_tl, 0) > 0 then dkd_alias_cargo_shipment.courier_fee_tl
          else coalesce(public.dkd_courier_jobs.fee_tl, 0)
        end
    ...
    payment_status = coalesce(nullif(trim(coalesce(dkd_alias_cargo_shipment.payment_status, '')), ''), 'paid'),
    paid_at = coalesce(dkd_alias_cargo_shipment.paid_at, dkd_alias_cargo_shipment.created_at, now())
from public.dkd_courier_jobs
where public.dkd_courier_jobs.cargo_shipment_id = dkd_alias_cargo_shipment.id;

update public.dkd_courier_jobs
set customer_charge_tl = case
    ...
    cargo_meta = coalesce(public.dkd_courier_jobs.cargo_meta, '{}'::jsonb)
      || jsonb_build_object(
        ...
        'cargo_courier_fee_tl', coalesce(public.dkd_courier_jobs.fee_tl, 0)
      )
...
begin
  ...
  dkd_fee_tl := public.dkd_cargo_delivery_fee_from_distance_km(dkd_delivery_distance_km);
...
begin
  ...
  where not exists (
    ...
    where public.dkd_courier_wallet_ledger.user_id = dkd_user_id
      and public.dkd_courier_wallet_ledger.related_job_id = dkd_param_job_id
      and coalesce(public.dkd_courier_wallet_ledger.kind, '') = 'job_reward'
  );
  ...
  if dkd_cargo_shipment_id is not null then
    ...
    set status = 'completed',
        completed_at = coalesce(completed_at, now()),
        courier_eta_min = 0,
        courier_fee_tl = case when coalesce(public.dkd_cargo_shipments.courier_fee_tl, 0) > 0 then public.dkd_cargo_shipments.courier_fee_tl else dkd_fee_tl end,
        customer_charge_tl = case when coalesce(public.dkd_cargo_shipments.customer_charge_tl, 0) > 0 then public.dkd_cargo_shipments.customer_charge_tl else public.dkd_cargo_customer_charge_from_courier_fee(dkd_fee_tl) end,
        payment_status = coalesce(nullif(trim(coalesce(public.dkd_cargo_shipments.payment_status, '')), ''), 'paid'),
        paid_at = coalesce(public.dkd_cargo_shipments.paid_at, public.dkd_cargo_shipments.created_at, now()),
        updated_at = now()
