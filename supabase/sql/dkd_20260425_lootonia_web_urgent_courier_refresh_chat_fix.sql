    ...
    'dkd_message_values', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'dkd_sender_display_name', case
          when dkd_message_row.dkd_sender_role_key = 'dkd_customer' then coalesce(nullif(trim(coalesce(dkd_order_row.dkd_customer_full_name, '')), ''), nullif(trim(coalesce(dkd_customer_profile_row.nickname, '')), ''), 'Müşteri')
          when dkd_message_row.dkd_sender_role_key = 'dkd_courier' then coalesce(nullif(trim(coalesce(dkd_sender_profile_row.nickname, '')), ''), nullif(trim(coalesce(dkd_courier_profile_row.nickname, '')), ''), 'Kurye')
          else 'Lootonia'
        end,
      ...
      left join public.dkd_profiles dkd_sender_profile_row
        on dkd_sender_profile_row.user_id = dkd_message_row.dkd_sender_user_id
