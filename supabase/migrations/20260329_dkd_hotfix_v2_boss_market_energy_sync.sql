-- Lootonia Hotfix V2
-- Boss HP kolonunu güvenli ekler
-- dkd_admin_boss_upsert fonksiyonunu çakışmasız yeniden oluşturur
-- Token mağazasına enerji paketlerini güvenli ekler
...
create function public.dkd_market_token_shop_buy(dkd_param_kind text)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select
    ...
    greatest(coalesce(energy_max, 20), 1)
  into
    dkd_token,
    dkd_shards,
    dkd_tickets,
  ...
  set
    ...
    energy = least(greatest(coalesce(energy_max, 20), 1), coalesce(energy, 0) + dkd_reward_energy),
    energy_max = greatest(coalesce(energy_max, 20), 1),
