...
-- Lootonia restore patch
-- Amaç:
-- 1) İşletme panelindeki marketi eski tasarım çizgisinde çalıştırmak
-- 2) Ana markette işletme ürünlerini token ile göstermek / satmak
-- 3) Önceki çakışan function sürümlerini güvenli şekilde temizlemek
...
as $$
  ...
  order by dkd_alias_b.name asc, coalesce(dkd_alias_p.sort_order, 0) asc, dkd_alias_p.updated_at desc;
$$;
...
drop function if exists public.dkd_business_product_buy_with_token(bigint);
drop function if exists public.dkd_business_product_buy_with_token(bigint, integer);
